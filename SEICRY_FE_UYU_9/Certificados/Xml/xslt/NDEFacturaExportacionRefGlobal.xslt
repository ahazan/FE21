      ...
      <ns1:eFact_Exp>
        ...
        <ns1:Encabezado>
          ...
          <ns1:IdDoc>
            ...
            <ns1:ClauVenta>
              <xsl:value-of select="ClausulaVenta"/>
            </ns1:ClauVenta>

            <xsl:if test="ModalidadVentaInt > 0">
              <ns1:ModVenta>
                <xsl:value-of select="ModalidadVentaInt"/>
              </ns1:ModVenta>
            </xsl:if>
            
            <ns1:ViaTransp>
              <xsl:value-of select="ViaTransporteInt"/>
            </ns1:ViaTransp>
          ...
          <ns1:Receptor>
            ...
            <xsl:if test="TipoDocumentoReceptorInt = 2 or TipoDocumentoReceptorInt = 3">
              <ns1:DocRecep>
                <xsl:value-of select="NumDocReceptor"/>
              </ns1:DocRecep>
            </xsl:if>

            <xsl:if test="TipoDocumentoReceptorInt = 4 or TipoDocumentoReceptorInt = 5 or TipoDocumentoReceptorInt = 6">
              <ns1:DocRecepExt>
                <xsl:value-of select="NumDocReceptorExtrangero"/>
              </ns1:DocRecepExt>
            </xsl:if>
          ...
          <ns1:Totales>
            ...
            <ns1:MntExpoyAsim>
              <xsl:value-of select="TotalMontoExportacionAsimilados"/>
            </ns1:MntExpoyAsim> 
        ...
        <xsl:for-each select="InfoReferencia/CFEInfoReferencia">
          ...
          <ns1:Referencia>
            ...
            <ns1:Referencia>
              ...
              <ns1:IndGlobal>
                <xsl:value-of select="IndicadorReferenciaGlobalInt"/>
              </ns1:IndGlobal>

              <ns1:RazonRef>
                <xsl:value-of select="RazonReferencia"/>
              </ns1:RazonRef>
