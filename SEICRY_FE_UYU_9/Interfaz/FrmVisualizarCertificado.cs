...
{
    class FrmVisualizarCertificado : FrmBase
    {
        ...
        {
            string formUID = "frmVisCer";
            Application app = SAPbouiCOM.Framework.Application.SBO_Application;

            //Se agrega un dataTable al formulario
            app.Forms.Item(formUID).DataSources.DataTables.Add("cerRec");
            //Se ejecuta una consulta para llenar el dataTable
            app.Forms.Item(formUID).DataSources.DataTables.Item("cerRec").ExecuteQuery(query);
            
            Grid grdCertificadosRechazados = (Grid)app.Forms.Item(formUID).Items.Item("grdCerRec").Specific;

            //Se llena el grid con la informacion del dataTable
            grdCertificadosRechazados.DataTable = app.Forms.Item(formUID).DataSources.DataTables.Item("cerRec");
